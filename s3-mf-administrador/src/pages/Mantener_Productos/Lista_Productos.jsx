  ...
  try {
    ...
    setProducts((prevProducts) =>
      prevProducts.map((product) =>
        product.product_id === currentProduct.product_id ? { ...product, active: newStatus } : product
      )
    );
    ...
    />
        ...
        <main className="flex-1 p-6 bg-gray-50">
          <div className="flex justify-between items-center">
            <h1 className="text-2xl font-bold text-red-700 text-center flex-grow">LISTA DE PRODUCTOS</h1>
              ...
              {filteredProducts.map((product) => (
                <div key={product.product_id} className="p-6 bg-white rounded-lg shadow-md">
                  ...
                  <button
                    ...
                    className={`mt-4 px-4 py-2 text-white rounded-lg ${product.active ? 'bg-red-700' : 'bg-gray-400'}`}>
       ...
       {showConfirmModal && (
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg max-w-lg">
          <h2 className="text-xl font-bold text-gray-700 mb-4 text-center">Confirmación</h2>
          <p className="mb-4 break-words text-lg" style={{ color: '#8C1C13' }}>
            ¿Seguro que desea {currentProduct.active ? 'deshabilitar' : 'habilitar'} el producto seleccionado "{currentProduct.product_name}"?
          </p>
          <div className="flex justify-center space-x-4 mt-4"> {/* Cambié justify-end por justify-center */}
            <button onClick={toggleProductStatus} className="px-4 py-2 text-white rounded-lg" style={{ backgroundColor: '#B60707' }}>
              Sí
            </button>
            <button onClick={() => setShowConfirmModal(false)} className="px-4 py-2 text-white rounded-lg" style={{ backgroundColor: '#4B4F57' }}>
              No
            </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
          <div className="bg-white rounded-lg p-8 shadow-lg w-96 relative">
            ...
            <form className="space-y-4 text-center" onSubmit={handleSubmit}>
              ...
              <button type="submit" className="w-full py-2 text-white bg-red-700 rounded-lg">Guardar</button>
            </form>
          </div>
        </div>
      )}
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
          <div className="bg-white rounded-lg p-8 shadow-lg w-96">
            ...
            <button onClick={() => setShowSuccessModal(false)} className="mt-4 w-full py-2 text-white bg-red-700 rounded-lg">Volver</button>
          </div>
        </div>
      )}
    </div>
